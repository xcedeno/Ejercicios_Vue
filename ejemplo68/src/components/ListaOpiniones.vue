<template>
    <div class="container">
        ...
        <div v-if="enviandoOpiniones.length > 0" data-cy="opinionesVisible">
            <div class="accordion" id="accordionExample">
                <div class="accordion-item mt-4" v-for="(item, index) in enviandoOpiniones" :key="index">
                    <h2 class="accordion-header" :id="'headingOne'+index">
                        <button class="accordion-button" type="button" data-bs-toggle="collapse" :data-bs-target="'#collapseOne'+index" aria-expanded="true" :aria-controls="'collapseOne'+index">
                            Opinión creada por: <span :data-cy="'nombre'+index">{{item.nombre}}</span>. Para el juego: {{item.juego.name}} 
                        </button>
                    </h2>
        ...
        <div class="alert alert-danger mt-5" role="alert" v-else data-cy="noOpiniones">
