<template>
  <div class="container">
    <div v-if="juego">
      <h2 class="text-center mt-5">Editando la opinión de: {{juego.juego.name}}</h2>
      <form>
        <div class="mb-3">
          <label for="nombre" class="form-label">Nombre:</label>
          <input type="text" class="form-control" id="nombre" v-model="juego.nombre">
        </div>
        <div class="mb-3">
          <label for="opiniones" class="form-label">Opiniones</label>
          <textarea class="form-control" id="opiniones" rows="3" v-model="juego.opinion"></textarea>
        </div>
...
export default {
  ...
  created() {
    let resultado = this.$store.getters.enviandoOpiniones.find(result => result.juego.id === this.id);
